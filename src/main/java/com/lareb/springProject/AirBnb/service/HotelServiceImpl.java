...
//        }
    ...
    public HotelInfoDto getInfoById(Long hotelId) {
        Hotel hotel = hotelRepository
                .findById(hotelId)
                .orElseThrow(()-> new ResourceNotFoundException("Hotel not found with ID: "+ hotelId));
        List<RoomDto> rooms = hotel.getRooms().stream().map((element)->
                modelMapper.map(element,RoomDto.class)).toList();
...
//if you try to create an inventory fot the days or roomms that already has an inventory then it will throw an exception
//
